 ...
 * @return 
 */
int baldns_separation_proc(arma::mat &mat, arma::vec &x, arma::vec &s,
						   std::unordered_set<int> &tSet, std::unordered_set<int> &wSet,
						   double zUpp, double zLow)
{
	int colIdx;
	int rowIdx;
	size_t i;
	...
	while (true)
	{
		...
		//std::cout << "M = "; for (auto it = mSetPtr->cbegin(); it != mSetPtr->cend(); ++it) std::cout << *it << " "; std::cout << std::endl;
...
double baldns_heur_dual_1(arma::mat &mat, arma::vec &x, arma::vec &u,
						  arma::vec &s, std::unordered_set<int> &tSet)
{
	bool firstTime;
	size_t i;
	size_t j;
						  ...
						  const double zUpp, double zLow)
{
	...
	while (prod < (zUpp - zLow - SC_EPSILON_SMALL))
	{
		rowIdx = rSetPtr->back().second;
		rSetPtr->pop_back();

		val = u(rowIdx);
		s += val * mat.row(rowIdx).t();
		prod += val * arma::dot(mat.row(rowIdx), x);
